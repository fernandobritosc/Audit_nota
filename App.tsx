...
// FIX: Removed unused KeyIcon import after removing the API key prompt.
import { LogoIcon, AlertTriangleIcon, PrinterIcon } from './components/icons';

// FIX: Removed obsolete ApiKeyPrompt component to enforce using API key from environment variables.
...
const App: React.FC = () => {
  ...
  // FIX: Removed state related to manual API key management to comply with guidelines.

  useEffect(() => {
    try {
      // FIX: Removed API key loading logic from session storage. The key is now handled exclusively in the service layer via environment variables.
  ...
  // FIX: Removed handleApiKeySubmit as manual key entry is no longer supported.
  ...
  const handleBatchAnalysis = useCallback(async () => {
    ...
    // FIX: Removed API key check, as it is now handled by the environment.
    ...
    for (const [index, file] of files.entries()) {
      ...
      try {
        ...
        // FIX: Call `extractInvoiceData` without passing the API key.
        ...
        // FIX: Simplified error handling as API key state is no longer managed in the UI.
  ...
  const handleReset = () => {
    ...
    setError(null);
    setIsLoading(false);
    setMode('analysis'); // Sempre retorna para a tela inicial padrão
  };

  // FIX: Removed conditional rendering for API key prompt.
