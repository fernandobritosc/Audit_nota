...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkApiKey = async () => {
      // In a real environment, window.aistudio might not be available immediately.
      // This is a simple check, could be improved with polling if needed.
      if (window.aistudio) {
        try {
         ...
         // If aistudio is not available, assume a legacy environment or local setup where process.env is managed differently.
         // For local dev without the platform, this allows the app to proceed.
         // The error will be caught later if API_KEY is truly missing.
         setApiKeyReady(true);
         setCheckingApiKey(false);
      }
    };
