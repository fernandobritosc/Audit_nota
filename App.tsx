...
const App: React.FC = () => {
  ...
  const handleTaxStatusChange = useCallback((key: 'optanteSimples' | 'isMei') => {
    ...
    const processedData = processTaxCalculations(updatedDataForRecalc as unknown as ExtractedData);

    setResult({
      ...updatedDataForRecalc,
      ...processedData,
    });
  }, [result]);

  const handleValueChange = useCallback((key: 'codigoReinf' | 'aliquotaIR', value: string | number) => {
    if (!result) return;
    ...
    if (key === 'aliquotaIR') {
        tempExtractedData.aliquotaIR = value as number;
    }

    const processedData = processTaxCalculations(tempExtractedData as unknown as ExtractedData);
      ...
      <div className="w-full max-w-2xl mx-auto print:max-w-none print:w-full print:p-8">
        ...
        <main className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200/80 w-full print:p-0 print:shadow-none print:border-none">
          <>
            ...
            {isLoading && (
              <div className="flex flex-col items-center justify-center text-center space-y-4 py-8 no-print">
                  <Spinner />
                  <p className="text-slate-600 font-medium">Analisando documento...</p>
                  <p className="text-sm text-slate-500">A IA está extraindo os dados e fazendo os cálculos.</p>
              </div>
            )}
