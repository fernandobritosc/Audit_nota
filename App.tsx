...
import { LogoIcon, AlertTriangleIcon, PrinterIcon, LogOutIcon } from './components/icons';
...
const App: React.FC = () => {
  ...
  // Hybrid API Key Management: Prefers environment variable for production, falls back to localStorage for local dev.
  const isProduction = !!process.env.API_KEY;
  ...
  const handleBatchAnalysis = useCallback(async () => {
    if (!apiKey) {
      ...
      if (!isProduction) handleChangeKey(); // Force re-entry only in local dev
      return;
    }
    ...
    for (const [index, file] of files.entries()) {
      ...
      try {
        ...
        // If the error is about authentication, show the API key screen again for local dev
        if (errorMessage.includes('autenticação') || errorMessage.includes('chave da API')) {
            setApiKeyError(errorMessage);
            if (!isProduction) {
                handleChangeKey(); // This will un-render the main app and show the key screen
            } else {
                setError(errorMessage); // In production, just show the error
            }
        } else {
  ...
  // Render ApiKeyEntryScreen if no key is available (only happens in local dev)
  if (!apiKey) {
