...
interface ResultsCardProps {
  ...
  onValueChange: (key: 'codigoReinf' | 'aliquotaIR' | 'aliquotaISS', value: string | number) => void;
}
...
const ResultsCard: React.FC<ResultsCardProps> = ({ data, onTaxStatusChange, onValueChange }) => {
    ...
    const calculoItems = [
        ...
        { label: `(-) Retenção I.R. (${data.irrf.rate.toString().replace('.',',')}%)`, value: formatCurrency(data.irrf.value), className: data.irrf.value > 0 ? 'text-red-600' : 'text-slate-500', observation: data.irrf.observacao },
    ];
    ...
    calculoItems.push(
        { label: `(-) Retenção ISS (${data.iss.rate.toString().replace('.',',')}%)`, value: formatCurrency(data.iss.value), className: data.iss.value > 0 ? 'text-red-600' : 'text-slate-500', observation: data.issObservacao },
