...
// FIX: Per @google/genai guidelines, remove apiKey parameter and use process.env.API_KEY.
export async function extractInvoiceData(base64DataUrl: string): Promise<ExtractedData> {
    // FIX: Per @google/genai guidelines, API key is assumed to be in process.env.API_KEY.
    ...
    try {
        ...
        console.error("Erro ao chamar a API do Gemini:", error);
         if (error instanceof Error) {
            if (error.message.includes('API key not valid')) {
                 // FIX: Per @google/genai guidelines, provide a generic error. Do not prompt for key.
        ...
        // FIX: Per @google/genai guidelines, simplified error message.
