...
export async function extractInvoiceData(base64DataUrl: string): Promise<ExtractedData> {
    // Access the API key using import.meta.env for Vite projects.
    // The '(import.meta as any)' is used to bypass potential TypeScript errors
    // if the Vite client types are not configured in tsconfig.json.
    const apiKey = (import.meta as any).env.VITE_API_KEY;
    if (!apiKey) {
      throw new Error("A chave da API (VITE_API_KEY) não foi encontrada. Verifique se ela está configurada corretamente nas variáveis de ambiente do seu projeto na Vercel.");
    }
