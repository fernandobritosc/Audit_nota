...
// FIX: Removed apiKey parameter. The API key must be sourced from environment variables.
export async function extractInvoiceData(base64DataUrl: string): Promise<ExtractedData> {
    // FIX: Initialize with API_KEY from process.env, which is a hard requirement.
